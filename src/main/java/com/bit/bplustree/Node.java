...
public class Node {
    ...
    /** 节点的关键字 */
    protected List<Entry<Comparable, Long>> entries;
    ...
    public Node(boolean isLeaf) {
        this.isLeaf = isLeaf;
        entries = new ArrayList<Entry<Comparable, Long>>();
    ...
    public void insertOrUpdate(Comparable key, Long value, BplusTree tree){
        //如果是叶子节点
        if (isLeaf){
            //不需要分裂，直接插入或更新
            ...
            if (key.compareTo(entries.get(0).getKey()) <= 0) {
                ...
                for (int i = 0; i < entries.size(); i++) {
                    if (entries.get(i).getKey().compareTo(key) <= 0 && entries.get(i+1).getKey().compareTo(key) > 0) {
                        children.get(i).insertOrUpdate(key, value, tree);
                        break;
                    }
                }
            }
            ...
            if (isRoot) {
                ...
                if (entries.size() > tree.getOrder() / 2 && entries.size() > 2) {
                    ...
                    if (previous != null
                        ...
                        Entry<Comparable, Long> entry = previous.getEntries().get(size - 1);
                        ...
                        Entry<Comparable, Long> entry = next.getEntries().get(0);
    ...
    /** 插入到当前节点的关键字中*/
    protected void insertOrUpdate(Comparable key, Long value){
        Entry<Comparable, Long> entry = new SimpleEntry<Comparable, Long>(key, value);
        //如果关键字列表长度为0，则直接插入
        if (entries.size() == 0) {
            entries.add(entry);
            return;
    ...
    public List<Entry<Comparable, Long>> getEntries() {
        return entries;
    }

    public void setEntries(List<Entry<Comparable, Long>> entries) {
